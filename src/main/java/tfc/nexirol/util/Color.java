...
public class Color {
	...
	private static double lerp(double amt, double start, double end) {
		return start * amt + (end * (1 - amt));
	}
	...
	public Color brighter(float amt, float hueShiftScale) {
		...
		r = (int) lerp(amt, r, 255);
		g = (int) lerp(amt, g, 255);
		b = (int) lerp(amt, b, 255);
		
		Color brighter = new Color(r, g, b, alpha);
