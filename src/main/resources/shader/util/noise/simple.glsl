...
float _perlin_noise_random_(vec2 coord, vec2 seed) {
...
float simpleNoise(vec2 p, float freq, vec2 seed) {
    ...
    float a = _perlin_noise_random_((ij + vec2(0., 0.)), seed);
    float b = _perlin_noise_random_((ij + vec2(1., 0.)), seed);
    float c = _perlin_noise_random_((ij + vec2(0., 1.)), seed);
    float d = _perlin_noise_random_((ij + vec2(1., 1.)), seed);
