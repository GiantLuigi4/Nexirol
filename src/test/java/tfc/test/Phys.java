...
//            PhysicsWorld world = new BulletWorld();
            ...
            final int MAX_INSTANCES = 1000;
            final int SHADER_MAX_INSTANCES = 1000;
            ...
            for (int i = 0; i < (MAX_INSTANCES - 1); i++) {
                ...
                world.addBody(new RigidBody(
                        ...
                        (float) (Math.random() * 25f - (25f / 2)) + finalI,
                        (float) (Math.random() * 25f - (25f / 2))
                ).onUpdate((body) -> {
                    // position
                    ...
                    // orientation
                    Shaders.cubeInstanceData.set(
                            finalI + MAX_INSTANCES, body.quat
                    );
                }));
                // position
                Shaders.cubeInstanceData.set(
                ...
                // orientation
                Shaders.cubeInstanceData.set(
                        finalI + MAX_INSTANCES, 0, 0, 0, 1
                );

                // scale
                Shaders.cubeInstanceData.set(
                        finalI + (MAX_INSTANCES * 2),
                        size, size, size
                );
                // color
                Shaders.cubeInstanceData.set(
                        finalI + (MAX_INSTANCES * 3), (float) Math.random(), (float) Math.random(), (float) Math.random(), 1
                );
                System.out.println(i + "/" + MAX_INSTANCES);
                collection.add(buffer, cube);
            }
            {
                int finalI = (MAX_INSTANCES - 1);
                world.addBody(new RigidBody(
...
//                    model.translate(0, 0, -50);
                ...
                }
                {
                    ...
                    world.tick();
                    Shaders.cubeInstanceData.upload(buffer);
